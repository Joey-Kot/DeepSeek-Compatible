...
func TestCreateResponseEndpointStoresAndRetrieves(t *testing.T) {
	...
	body := `{"model":"deepseek-v4-pro","instructions":"Be brief.","input":"Hello"}`
...
func TestResponsesInputTokensUsesLocalTokenizer(t *testing.T) {
	...
	if data["object"] != "response.input_tokens" || int(data["input_tokens"].(float64)) <= 0 {
		t.Fatalf("input tokens response = %#v", data)
	}
}

func TestDeepSeekChatCompletionPassthrough(t *testing.T) {
	server := testServer(fakeUpstream{chatFn: func(payload shared.Map) (shared.Map, error) {
		...
		if payload["custom_deepseek_field"] != "kept" {
	...
	body := `{"model":"deepseek-v4-pro","messages":[{"role":"user","content":"Hello"}],"max_tokens":8,"temperature":0.2,"custom_deepseek_field":"kept"}`
	rec := request(server, http.MethodPost, "/chat/completions", body)
	if rec.Code != http.StatusOK {
		t.Fatalf("status=%d body=%s", rec.Code, rec.Body.String())
	}
	if !strings.Contains(rec.Body.String(), `"id":"deepseek_1"`) || !strings.Contains(rec.Body.String(), `"content":"Hi!"`) {
		t.Fatalf("body=%s", rec.Body.String())
	}
}

func TestDeepSeekChatCompletionStreamPassthrough(t *testing.T) {
	server := testServer(fakeUpstream{streamFn: func(payload shared.Map, handle func(shared.Map) error) error {
		if payload["stream"] != true {
			t.Fatalf("stream=%v", payload["stream"])
		}
...
func TestCreateChatCompletionStoresAndLists(t *testing.T) {
	...
	body := `{"model":"deepseek-v4-pro","messages":[{"role":"user","content":"Hello"}],"max_tokens":8,"metadata":{"topic":"demo"},"store":true}`
	...
	if rec.Code != http.StatusOK || !strings.Contains(rec.Body.String(), "deepseek_chat_1") {
		t.Fatalf("list status=%d body=%s", rec.Code, rec.Body.String())
	}
	rec = request(server, http.MethodGet, "/v1/chat/completions/deepseek_chat_1/messages", "")
...
func TestAnthropicMessageEndpointUsesXAPIKey(t *testing.T) {
	...
	body := `{"model":"deepseek-v4-pro","system":"Be brief.","max_tokens":8,"messages":[{"role":"user","content":"Hello"}]}`
...
func TestAnthropicCountTokens(t *testing.T) {
	server := testServer(fakeUpstream{})
	body := `{"model":"deepseek-v4-pro","messages":[{"role":"user","content":"Hello world"}]}`
...
func TestDebugLogBodyLogsRedactedRequestAndResponse(t *testing.T) {
	...
	body := `{"model":"deepseek-v4-pro","api_key":"secret-value","messages":[{"role":"user","content":"Hello"}]}`
	rec := request(server, http.MethodPost, "/v1/chat/completions", body)
	if rec.Code != http.StatusOK {
		t.Fatalf("status=%d body=%s", rec.Code, rec.Body.String())
	}
