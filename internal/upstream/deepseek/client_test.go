...
func TestChatPostsJSONAndParsesResponse(t *testing.T) {
	...
	response, err := client.Chat(context.Background(), shared.Map{"model": "deepseek-v4-pro"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestChatUsesExplicitChatCompletionsURL(t *testing.T) {
	client := New("https://deepseek.test/custom/chat/completions", "sk-upstream", time.Second, true)
	client.HTTPClient = &http.Client{Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
		if r.URL.String() != "https://deepseek.test/custom/chat/completions" {
			t.Fatalf("url = %s", r.URL.String())
		}
...
func TestDebugLogBodyLogsRedactedUpstreamBodies(t *testing.T) {
	...
	_, err := client.Chat(context.Background(), shared.Map{"model": "deepseek-v4-pro", "api_key": "secret-value"})
	if err != nil {
		t.Fatal(err)
	}
	text := logs.String()
	if !strings.Contains(text, "debug body deepseek request") || !strings.Contains(text, "debug body deepseek response") {
...
func TestNewConfiguresTLSVerification(t *testing.T) {
	verified := New("https://deepseek.test", "sk-upstream", time.Second, true)
	if isInsecureSkipVerify(verified.HTTPClient) {
		t.Fatal("expected TLS verification to be enabled by default")
	}

	unverified := New("https://deepseek.test", "sk-upstream", time.Second, false)
