...
func (a Adapter) BuildDeepSeekPayload(payload shared.Map) (Prepared, error) {
...
func ContentToChatMessages(role string, content any) []shared.Map {
	...
	for _, raw := range blocks {
		...
		case "image", "document", "search_result", "web_search_tool_result", "web_fetch_tool_result", "code_execution_tool_result":
...
func DescribeUnsupportedBlock(block map[string]any) string {
	...
	return "\n[" + t + " block omitted by DeepSeek compatibility layer: " + shared.JSONString(block) + "]\n"
}
...
func NormalizeTools(value any) []shared.Map {
	...
	for _, raw := range rawTools {
		...
		function := shared.Map{
			"name":        shared.SafeDeepSeekToolName(name),
...
func ResponseFromDeepSeek(completion shared.Map, requestPayload shared.Map, defaultModel string) shared.Map {
...
func UsageFromDeepSeek(value any) shared.Map {
