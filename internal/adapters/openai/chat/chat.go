...
func (a Adapter) BuildDeepSeekPayload(payload shared.Map) (shared.Map, []shared.Map, error) {
...
func NormalizeFunctionTool(function map[string]any) shared.Map {
	...
	normalized["name"] = shared.SafeDeepSeekToolName(shared.StringValue(normalized["name"]))
...
func NormalizeTool(tool any) shared.Map {
	...
	case "custom":
		...
		return shared.Map{
			"type": "function",
			"function": shared.Map{
				"name":        shared.SafeDeepSeekToolName(shared.StringValue(custom["name"])),
		...
		case "function":
			if function, ok := choice["function"].(map[string]any); ok {
				if name := shared.StringValue(function["name"]); name != "" {
					return shared.Map{"type": "function", "function": shared.Map{"name": shared.SafeDeepSeekToolName(name)}}
				}
			}
		case "custom":
...
func CompletionFromDeepSeek(completion shared.Map, requestPayload shared.Map, defaultModel string) shared.Map {
	...
	return shared.Map{
		...
		"model":              model,
		"choices":            choices,
		"usage":              UsageFromDeepSeek(completion["usage"]),
...
func UsageFromDeepSeek(value any) any {
...
func SSEData(chunk shared.Map, requestPayload shared.Map, defaultModel string) shared.Map {
	...
	if data["usage"] != nil {
		data["usage"] = UsageFromDeepSeek(data["usage"])
	}
	return data
}
...
func filterAllowedTools(tools []shared.Map, choice map[string]any) []shared.Map {
	...
	for _, entry := range raw {
		...
		if name != "" {
			allowedNames[shared.SafeDeepSeekToolName(name)] = true
		}
	}
