...
func TestSafeAndUniqueDeepSeekToolName(t *testing.T) {
	if got := SafeDeepSeekToolName("mcp tool.with spaces"); got != "mcp_tool_with_spaces" {
		t.Fatalf("SafeDeepSeekToolName = %q", got)
	}
	long := strings.Repeat("a", 100)
	if got := SafeDeepSeekToolName(long); len(got) > MaxDeepSeekToolNameLength {
		t.Fatalf("long tool name was not capped: %q", got)
	}
	used := map[string]string{}
	first := UniqueDeepSeekToolName("tool/a", "", used)
	second := UniqueDeepSeekToolName("tool a", "", used)
	if first == second {
		t.Fatalf("colliding raw names produced same deepseek name: %q", first)
	}
}
