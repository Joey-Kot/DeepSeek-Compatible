...
import (
	...
	"deepseek-responses-compatible/internal/tokenizer/deepseekv3"
)
...
const MaxDeepSeekToolNameLength = 64
...
func RawDeepSeekToolName(name any, namespace string) string {
...
func SafeDeepSeekToolName(raw string) string {
	safe := strings.Trim(toolNameUnsafe.ReplaceAllString(raw, "_"), "_")
	if safe == "" {
		safe = "tool"
	}
	if len(safe) <= MaxDeepSeekToolNameLength {
		return safe
	}
	digest := shortHash(raw, 12)
	prefixLen := MaxDeepSeekToolNameLength - len(digest) - 1
	return strings.TrimRight(safe[:prefixLen], "_-") + "_" + digest
}

func UniqueDeepSeekToolName(name any, namespace string, used map[string]string) string {
	raw := RawDeepSeekToolName(name, namespace)
	candidate := SafeDeepSeekToolName(raw)
	...
	for suffix := 2; ; suffix++ {
		...
		s := fmt.Sprint(suffix)
		prefixLen = MaxDeepSeekToolNameLength - len(digest) - len(s) - 2
		base = candidate
		if len(base) > prefixLen {
			base = base[:prefixLen]
		}
...
func EstimateTokensFromMessages(messages []Map) int {
	if count := countTokensWithDeepSeekTokenizer(messages); count > 0 {
		return count
	}
	return estimateTokensFromMessages(messages)
}

func countTokensWithDeepSeekTokenizer(messages []Map) int {
	if len(messages) == 0 {
		return 0
	}
	rendered := make([]deepseekv3.Message, 0, len(messages))
	for _, message := range messages {
		rendered = append(rendered, deepseekv3.Message{
			Role:      StringValue(message["role"]),
			Content:   ContentToText(message["content"], true),
			ToolCalls: tokenizerToolCalls(message["tool_calls"]),
		})
	}
	return deepseekv3.CountMessages(rendered)
}

func tokenizerToolCalls(value any) []deepseekv3.ToolCall {
	raw, ok := value.([]any)
	if !ok {
		return nil
	}
	out := []deepseekv3.ToolCall{}
	for _, item := range raw {
		...
		out = append(out, deepseekv3.ToolCall{
