	...
	for i := 0; i < len(items); i++ {
		...
		case itemType == "message":
			if shared.StringValue(item["role"]) == "assistant" {
				...
				for ; j < len(items); j++ {
					candidate := items[j]
					if !IsToolCallItem(candidate) || (item["_deepseek_turn_id"] != nil && !SameDeepSeekTurn(item, candidate)) {
						break
					}
					calls = append(calls, candidate)
				}
				if len(calls) > 0 {
					messages = append(messages, AssistantToolCallMessage(calls, shared.ContentToText(item["content"], true), shared.StringValue(item["_deepseek_reasoning_content"])))
					i = j - 1
					continue
				}
			}
		...
		case IsToolCallItem(item):
			...
			for ; j < len(items); j++ {
				candidate := items[j]
				if !IsToolCallItem(candidate) || (item["_deepseek_turn_id"] != nil && candidate["_deepseek_turn_id"] != nil && !SameDeepSeekTurn(item, candidate)) {
					break
				}
				calls = append(calls, candidate)
			}
...
func ResponseMessageToChatMessage(item shared.Map) shared.Map {
	...
	message := shared.Map{"role": role, "content": shared.ContentToText(item["content"], role == "assistant")}
	if role == "assistant" && shared.StringValue(item["_deepseek_reasoning_content"]) != "" {
		message["reasoning_content"] = item["_deepseek_reasoning_content"]
	}
	return message
}
...
func SameDeepSeekTurn(left, right shared.Map) bool {
	l := shared.StringValue(left["_deepseek_turn_id"])
	r := shared.StringValue(right["_deepseek_turn_id"])
	return l != "" && r != "" && l == r
}
...
func AssistantToolCallMessage(calls []shared.Map, args ...string) shared.Map {
	...
	for _, call := range calls {
		...
		if reasoning == "" {
			reasoning = shared.StringValue(call["_deepseek_reasoning_content"])
		}
	}
...
func FunctionCallToToolCall(item shared.Map) shared.Map {
	...
	name := DeepSeekToolNameForResponseItem(item)
...
func DeepSeekToolNameForResponseItem(item shared.Map) string {
	if name := shared.StringValue(item["_deepseek_tool_name"]); name != "" {
		return name
	}
	return shared.SafeDeepSeekToolName(shared.RawDeepSeekToolName(item["name"], shared.StringValue(item["namespace"])))
}

func (a Adapter) BuildDeepSeekPayload(payload shared.Map, messages []shared.Map) (shared.Map, map[string]shared.Map) {
	...
	tools, toolNameMap := NormalizeToolsForDeepSeek(payload["tools"])
...
func NormalizeToolsForDeepSeek(value any) ([]shared.Map, map[string]shared.Map) {
...
func NormalizeFunctionTool(tool map[string]any, used map[string]string, toolNameMap map[string]shared.Map, namespace, namespaceDescription string) shared.Map {
	...
	deepseekName := shared.UniqueDeepSeekToolName(originalName, namespace, used)
	function["name"] = deepseekName
	...
	toolNameMap[deepseekName] = shared.Map{"type": "function", "name": originalName, "namespace": nilIfEmpty(namespace), "deepseek_name": deepseekName}
...
func NormalizeCustomTool(tool map[string]any, used map[string]string, toolNameMap map[string]shared.Map, namespace, namespaceDescription string) shared.Map {
	...
	toolNameMap[deepseekName] = shared.Map{"type": "custom", "name": originalName, "namespace": nilIfEmpty(namespace), "deepseek_name": deepseekName}
	return shared.Map{"type": "function", "function": function}
}
...
func FilterToolsForAllowedChoice(tools []shared.Map, choice map[string]any, toolNameMap map[string]shared.Map) []shared.Map {
	...
	for _, entry := range allowed {
		for deepseekName, mapping := range toolNameMap {
			if AllowedEntryMatches(entry, mapping) {
				allowedNames[deepseekName] = true
			}
		}
	}
...
func AllowedEntryMatches(entry any, mapping shared.Map) bool {
	if text, ok := entry.(string); ok {
		return text == mapping["name"] || text == mapping["deepseek_name"] || text == shared.RawDeepSeekToolName(mapping["name"], shared.StringValue(mapping["namespace"]))
	}
	...
	if name := shared.StringValue(e["name"]); name != "" && name != shared.StringValue(mapping["name"]) && name != shared.StringValue(mapping["deepseek_name"]) {
		return false
	}
...
func (a Adapter) OutputItemsFromChatCompletion(completion shared.Map, toolNameMap map[string]shared.Map) ([]shared.Map, string, string, []any) {
	...
	if content != "" {
		...
		if turnID != "" {
			item["_deepseek_turn_id"] = turnID
		}
		if reasoning != "" {
			item["_deepseek_reasoning_content"] = reasoning
		}
	...
	for index, raw := range toolCalls {
		...
		output = append(output, DeepSeekToolCallToResponseItem(toolCall, toolNameMap, callReasoning, turnID))
	}
...
func DeepSeekToolCallToResponseItem(toolCall map[string]any, toolNameMap map[string]shared.Map, reasoning, turnID string) shared.Map {
	function, _ := toolCall["function"].(map[string]any)
	arguments := shared.JSONString(function["arguments"])
	if arguments == "null" {
		arguments = ""
	}
	deepseekName := shared.StringValue(function["name"])
	if deepseekName == "" {
		deepseekName = "function"
	}
	mapping := toolNameMap[deepseekName]
	...
	item := shared.Map{"id": shared.NewID("fc"), "call_id": valueOrDefault(toolCall["id"], shared.NewID("call")), "name": name, "status": "completed", "_deepseek_tool_name": deepseekName}
	if itemType == "custom" {
		input := arguments
...
func ResponseUsageFromDeepSeek(value any) any {
...
func mappedToolNameForChoice(name any, namespace, toolType string, toolNameMap map[string]shared.Map) string {
	...
	for deepseekName, mapping := range toolNameMap {
		if shared.StringValue(mapping["name"]) != want && shared.StringValue(mapping["deepseek_name"]) != want {
			continue
		}
	...
	return shared.SafeDeepSeekToolName(shared.RawDeepSeekToolName(want, namespace))
}
