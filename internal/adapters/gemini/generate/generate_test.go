...
func TestBuildDeepSeekPayloadMapsGeminiGenerateContent(t *testing.T) {
	...
	prepared, err := adapter.BuildDeepSeekPayload("gemini-3.5-flash", payload)
	if err != nil {
		t.Fatal(err)
	}
...
func TestResponseFromDeepSeekMapsGeminiParts(t *testing.T) {
	...
	response := ResponseFromDeepSeek(completion, "gemini-3.5-flash")
