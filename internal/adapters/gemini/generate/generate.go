...
func (a Adapter) BuildDeepSeekPayload(model string, payload shared.Map) (Prepared, error) {
	if model == "" {
...
func DescribeUnsupportedPart(part map[string]any) string {
	return "\n[Gemini part omitted by DeepSeek compatibility layer: " + shared.JSONString(part) + "]\n"
}
...
func NormalizeToolChoice(value any, allowed map[string]bool) any {
	...
	for _, rawName := range namesList(fc["allowedFunctionNames"]) {
		allowed[shared.SafeDeepSeekToolName(rawName)] = true
	}
...
func ResponseFromDeepSeek(completion shared.Map, model string) shared.Map {
	if model == "" {
	...
	return shared.Map{
		...
		"usageMetadata": UsageFromDeepSeek(completion["usage"]),
		"modelVersion":  model,
...
func StreamChunkFromDeepSeek(chunk shared.Map, model string) shared.Map {
	...
	return ResponseFromDeepSeek(converted, model)
}
